...
@pytest.fixture
def task_1():
    return ActivityTask(domain='d', activity_id='t1:1:d', name='t1', version='1')

@pytest.fixture
def task_2():
    return ActivityTask(domain='d', activity_id='t2:1:d', name='t2', version='1')
...
class TestExecutionGraph():
    ...
    def test_graph_from_task_specs(self):
        t1 = ActivityTask(domain='d', activity_id='t1:1:d', name='t1', version='1')
        t2 = Timer(id_='t2', requires=[t1])
        g = floto.decider.ExecutionGraph(activity_tasks=[t1,t2])
        t1_idx = g.id_to_idx['t1:1:d']
    ...
    def test_transitive_reduction(self):
        t1 = ActivityTask(domain='d', activity_id='t1:1:d', name='t1', version='1')
        t2 = ActivityTask(domain='d', activity_id='t2:1:d', name='t2', version='1', requires=[t1])
        t3 = ActivityTask(domain='d', activity_id='t3:1:d', name='t3', version='1', requires=[t1, t2])
        
        g = floto.decider.ExecutionGraph(activity_tasks=[t1,t2,t3])
        t1_idx = g.id_to_idx['t1:1:d']
        t2_idx = g.id_to_idx['t2:1:d']
        t3_idx = g.id_to_idx['t3:1:d']
    ...
    def test_id_to_idx(self):
        tasks = [ActivityTask(domain='d', activity_id='t1:1:d', name='t1', version='1'),
                 ActivityTask(domain='d', activity_id='t2:1:d', name='t2', version='1')]
        g = floto.decider.ExecutionGraph(activity_tasks=tasks)
        assert g.id_to_idx['t1:1:d'] == 0
        assert g.id_to_idx['t2:1:d'] == 1

    def test_id_to_idx_raises_for_duplicate_activity_ids(self):
        tasks = [ActivityTask(domain='d', activity_id='t1:1:d', name='t1', version='1'),
                 ActivityTask(domain='d', activity_id='t1:1:d', name='t2', version='1')]
        g = floto.decider.ExecutionGraph(activity_tasks=tasks)
    ...
    def test_tasks_by_id(self):
        tasks = [ActivityTask(domain='d', activity_id='t1:1:d', name='t1', version='1')]
        g = floto.decider.ExecutionGraph(activity_tasks=tasks)
        assert g.tasks_by_id['t1:1:d'].name == 't1'
    ...
    def test_get_column_of_graph_matrix(self):
        ...
        tasks = [t1,t2]
        g = floto.decider.ExecutionGraph(activity_tasks=tasks)
        assert g._get_column_of_graph_matrix('t1:1:d') == [0,0]
        assert g._get_column_of_graph_matrix('t2:1:d') == [1,0]
    ...
    def test_get_first_tasks(self):
        ...
        assert g.get_first_tasks()[0].id_ == 't1:1:d' 

    def test_get_depending_tasks(self):
        t1 = ActivityTask(domain='d', activity_id='t1:1:d', name='t1', version='1')
        t2 = ActivityTask(domain='d', activity_id='t2:1:d', name='t2', version='1', requires=[t1])
        g = floto.decider.ExecutionGraph(activity_tasks=[t1,t2])
        depending_tasks = g.get_depending_tasks('t1:1:d')
        assert len(depending_tasks) == 1
        assert depending_tasks[0].id_ == 't2:1:d'

    def test_get_dependencies(self):
        t1 = ActivityTask(domain='d', activity_id='t1:1:d', name='t1', version='1')
        t2 = ActivityTask(domain='d', activity_id='t2:1:d', name='t2', version='1', requires=[t1])
        g = floto.decider.ExecutionGraph(activity_tasks=[t1,t2])
        dependencies = g.get_dependencies('t2:1:d')
        assert len(dependencies) == 1
        assert dependencies[0].id_ == 't1:1:d'

    def test_outgoing_vertices(self):
        t1 = ActivityTask(domain='d', activity_id='t1:1:d', name='t1', version='1')
        t2 = ActivityTask(domain='d', activity_id='t2:1:d', name='t2', version='1', requires=[t1])
        t3 = ActivityTask(domain='d', activity_id='t3:1:d', name='t3', version='1', requires=[t2])
        t4 = ActivityTask(domain='d', activity_id='t4:1:d', name='t4', version='1')
        g = floto.decider.ExecutionGraph(activity_tasks=[t1,t2,t3,t4])
        outgoing = g.outgoing_vertices()
        assert len(outgoing) == 2
        assert set([e.id_ for e in outgoing]) == set(['t3:1:d', 't4:1:d'])
    ...
    def test_update(self):
        ...
        t1 = ActivityTask(domain='d', activity_id='t1:1:d', name='t1', version='1')
        g = Generator(domain='d', activity_id='g:1:d', name='g', version='1', requires=[t1])
        t3 = ActivityTask(domain='d', activity_id='t3:1:d', name='t3', version='1', requires=[g])

        t4 = ActivityTask(domain='d', activity_id='t4:1:d', name='t4', version='1')
        t5 = ActivityTask(domain='d', activity_id='t5:1:d', name='t5', version='1')

        t6 = ActivityTask(domain='d', activity_id='t6:1', name='t6', version='1')

        graph = floto.decider.ExecutionGraph(activity_tasks=[t1,g,t3,t6])
        graph.update(g, [t4, t5])

        assert get_task_requires(g, graph.tasks) == set(['t1:1:d'])
        assert get_task_requires(t3, graph.tasks) == set(['t4:1:d', 't5:1:d'])
        assert get_task_requires(t4, graph.tasks) == set(['g:1:d'])
        assert get_task_requires(t5, graph.tasks) == set(['g:1:d'])
